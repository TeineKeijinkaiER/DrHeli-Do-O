...
最終更新: 2026-09-09 / 対象: 次セッションの作業者（どのAIでも可）
...
DO-O/
  pwa/                     ← アプリ本体(正本)
    ...
    data/{regions,inventory,beginner,quiz,reflection-lessons,stats,operating-hours}.json
...
- 選択肢は既定で畳んである（`details`）。未選択のときは「All」と出す。
- **初期表示は最新年度だけ**に絞る（80件を一度に出すと目的の年に辿り着けないため）。
  年は直書きせず `date` の最大値から決めるので、議事録が増えれば自動で追随する。
- 該当0件の選択肢は表示しない。
...
- 表示する指標は8つ: 要請件数 / 応需率 / 出動数 / 対応患者数 / 挿管 /
  覚知→接触の中央値 / 現場滞在時間の中央値 / 総出動時間の中央値。
  指標は絞り込みより**上**に置く(絞り込みが14軸あり、下だと数値までスクロールが長い)。
...
- **総出動時間**: ドクターヘリ離陸(`timtkfbh`)→基地病院帰投(`timldgbh`)。中央値63分
  (現場搬送61分 / 施設間搬送86分)。0〜360分の範囲外は捨てる。
...
- `sw.js`: CACHE名は版数管理(現在 **doo-heli-v29**)。**データJSON(/data/*.json)はネットワーク優先**、アプリ本体(html/js/css/img/vendor)はキャッシュ優先+背景更新。activateで `KEEP` 以外の旧キャッシュ削除。
...
- [x] 2026-09-09: 統計に**総出動時間**(ヘリ離陸→基地帰投・中央値63分)を追加。
      リフレクションの初期表示を最新年度のみに変更。SW v29。
...
- regions.json: 52市町村 / 58RP（実績中央値52・回帰6）。SW: doo-heli-v29 / タイル: doo-heli-tiles-v1。
